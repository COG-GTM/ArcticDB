...
static std::pair<TimeseriesDescriptor, std::optional<SegmentInMemory>> get_descriptor_and_data(
        const std::shared_ptr<StreamSource>& store, const AtomKey& k, bool load_data, storage::ReadKeyOpts opts
) {
    if (load_data) {
...
AppendMapEntry append_map_entry_from_key(
    ...
    auto frame_slice = pipelines::FrameSlice{
            desc, pipelines::ColRange{index_field_count, field_count}, pipelines::RowRange{0, entry.total_rows_}
    };
...
[[nodiscard]] folly::Future<std::vector<arcticdb::entity::AtomKey>> write_incomplete_frame_with_sorting(
    ...
    // Clone is insufficient for string columns in the input frame (Monday: 18190648152).
    ...
    bool is_sorted = is_timestamp_index &&
                     (options.sort_on_index || (is_timestamp_index && options.sort_columns &&
                                                options.sort_columns->at(0) == segment.descriptor().field(0).name()));
    ...
    auto res = std::visit(
            [&segments, &store, &stream_id, &norm_meta, &stream_desc, is_sorted](auto&& idx) {
                using IdxType = std::decay_t<decltype(idx)>;
...
[[nodiscard]] folly::Future<std::vector<arcticdb::entity::AtomKey>> write_incomplete_frame(
    ...
    write_options.column_group_size =
            std::numeric_limits<size_t>::max(); // column slicing not supported yet (makes it hard
...
std::vector<VariantKey> read_incomplete_keys_for_symbol(
    ...
    std::transform(
            entries.cbegin(),
            entries.cend(),
            std::back_inserter(slice_and_key),
...
std::variant<std::vector<SliceAndKey>, CompactionError> get_incomplete_segments_using_stage_results(
    ...
    util::check(
            ...
            "read_incompletes_to_pipeline with keys_to_read specified "
    ...
    user_input::check<ErrorCode::E_INVALID_USER_ARGUMENT>(
            flags.via_iteration,
            "read_incompletes_to_pipeline with keys_to_read specified and not via_iteration is not supported"
    );
    std::vector<AppendMapEntry> entries;
