...
class NeverSegmentPolicy {
  public:
    ...
    constexpr bool operator()(AggregationStats&) const { return false; }
    ...
    bool operator()(AggregationStats& stats) const {
        ...
        compile_for_(
                ...
                [&is_policy_valid, &stats](const auto& variant_policy) {
                    std::visit(
                            [&is_policy_valid, &stats](const auto& policy) {
...
class Aggregator {
  public:
    ...
    template<class C>
    Aggregator(
            SchemaPolicy&& schema, C&& c, SegmentingPolicy&& segmenting_policy = SegmentingPolicyType(),
        ...
        segment_(
                desc ? *desc : schema_policy_.default_descriptor(),
                row_count.value_or(segmenting_policy_.expected_row_size()), AllocationType::DYNAMIC,
    ...
    RowBuilderType& row_builder() { return row_builder_; }

    template<class... Args>
    RowBuilderType& start_row(Args... args) {
        SCOPE_FAIL { row_builder_.rollback_row(); };
    ...
    const IndexType& index() const { return std::get<IndexType>(schema_policy_.index()); }
    ...
    SegmentingPolicyType segmenting_policy() { return segmenting_policy_; }
  ...
  private:
    ...
    void set_scalar_by_name(std::string_view name, T val, DataType data_type) {
        position_t pos = schema_policy_.get_column_idx_by_name(
                segment_,
                name,
    ...
    void set_string(position_t pos, const std::string& str) { segment_.set_string(pos, str); }

    void set_string(position_t pos, std::string_view str) { segment_.set_string(pos, str); }

    void set_string_by_name(std::string_view name, std::string_view str, DataType desc) {
        position_t pos = schema_policy_.get_column_idx_by_name(
                segment_, name, make_scalar_type(desc), segmenting_policy_.expected_row_size(), segment_.row_count()
        );
        set_string(pos, str);
    }
