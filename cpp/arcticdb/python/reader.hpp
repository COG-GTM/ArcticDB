...
class TickReader {
  public:
    ...
    void add_segment(SegmentInMemory& segment) { segment_ = std::move(segment); }

    [[nodiscard]] size_t row_count() const { return segment_.row_count(); }

    std::string_view view_at(entity::position_t o) { return segment_.string_pool().get_view(o); }
