...
struct Arbitrary<arcticdb::entity::StreamDescriptor> {
    static Gen<arcticdb::entity::StreamDescriptor> arbitrary() {
        ...
        auto desc = stream_descriptor_from_range(
                arcticdb::StreamId{id}, arcticdb::stream::RowCountIndex{}, std::move(field_descriptors)
        );
...
struct Arbitrary<TestDataFrame> {
    static Gen<TestDataFrame> arbitrary() {
        ...
        return gen::build<TestDataFrame>(
                ...
                gen::set(
                        &TestDataFrame::types_,
                        gen::container<std::vector<arcticdb::entity::TypeDescriptor>>(
                                num_rows, rc::gen::arbitrary<arcticdb::entity::TypeDescriptor>()
                        )
                ),
                gen::set(
                        &TestDataFrame::data_,
                        gen::container<std::vector<std::vector<int>>>(
                                num_columns, gen::container<std::vector<int>>(num_rows, gen::inRange(0, 100))
                        )
                ),
                gen::set(&TestDataFrame::start_ts_, gen::inRange(1, 100)),
                gen::set(
                        &TestDataFrame::timestamp_increments_,
                        gen::container<std::vector<uint8_t>>(num_rows, gen::inRange(1, 100))
                ),
                gen::set(
                        &TestDataFrame::column_names_,
...
inline ac::IndexRange test_frame_range(const TestDataFrame& data_frame) {
    return ac::IndexRange{
            data_frame.start_ts_,
            std::accumulate(
                    data_frame.timestamp_increments_.begin(),
    ...
    reader.foreach_row([&row, &timestamp, &success, &data_frame, &errors](auto&& row_ref) {
        ...
        for (size_t col = 0; col < data_frame.num_columns_; ++col) {
            data_frame.types_[col].visit_tag([&](auto type_desc_tag) {
                ...
                arcticdb::entity::DataType stored_dt = row_ref.segment().column_descriptor(col + 1).type().data_type();
                if (dt != stored_dt) {
                ...
                if (dimension != static_cast<uint64_t>(stored_dimension)) {
                    errors.emplace_back(fmt::format(
                            "Dimension mismatch {} != {} at pos {}:{}", dimension, stored_dimension, col, row
                    ));
                    success = false;
                }
                ...
                if (value != stored_value) {
                    errors.push_back(fmt::format("Value mismatch {} != {} at pos {}:{}", value, stored_value, col, row)
                    );
                    success = false;
                }
            });
...
inline bool check_test_frame(
    ...
    ac::StreamReader<
            ...
            stream_reader{[&]() { return std::vector<arcticdb::entity::AtomKey>{key}; }, std::move(store)};
