        ...
        return visit_field(frame.field(col_pos), [](auto tag) {
            ...
            if constexpr (is_sequence_type(data_type)) {
                ...
                if constexpr (data_type == DataType::NANOSECONDS_UTC64) {
                    // Pandas < 2.0 always uses `datetime64[ns]`. Pandas >= 2.0 also supports [us], [ms], [s].
                    // See:
                    ...
                    // OPTIM: Support Pandas>=2.0 resolutions by converting datetime to datetime64[ns] beforehand
                    // rather than relying on the resolution-less 'M' specifier.
                    dtype = "datetime64[ns]";
                } else {
